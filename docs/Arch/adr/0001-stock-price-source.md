...
使用者回報持有多檔不同股票時，淨資產彙總（`GET /api/v1/net-worth`）仍然很慢——即使
`net_worth_service.py` 已改成對每檔股票平行發起報價查詢，`app/clients/stock_price_client.py`
的 `TwseMisClient` 內部有一個同一 client 實例共用的自我限速鎖（ADR-0001 決策段記載的
~1 req/2s 自限），任何呼叫都得排隊，所以「不同標的、同一來源」的查詢即使平行發起，實際對外
發送時仍會被這個鎖序列化，持有檔數越多、疊加等待越久。

重新測試 MIS 端點後發現：`ex_ch` 參數其實支援用 `|` 分隔**多個**值一次查詢，例如
`ex_ch=tse_2330.tw|tse_0050.tw|tse_1101.tw`，回應 `msgArray` 依請求順序逐一對應每個
`ex_ch` 值（包含查無資料的股號——用錯市場前綴時，該位置回跟單檔查詢一樣的空殼物件
`{"tv":"-","s":"-","c":"","z":"-"}`，陣列長度與請求數一致）。以下為當次真實 `curl` 輸出
（2026-09-18）：

```
GET https://mis.twse.com.tw/stock/api/getStockInfo.jsp?ex_ch=tse_2330.tw|tse_0050.tw|tse_1101.tw&json=1&delay=0

{"msgArray":[
  {"@":"2330.tw","c":"2330","n":"台積電","z":"2460.0000", ...},
  {"@":"0050.tw","c":"0050","n":"元大台灣50","z":"109.8500", ...},
  {"@":"1101.tw","c":"1101","n":"台泥","z":"24.3500", ...}
], "rtcode":"0000","rtmessage":"OK", ...}
```

混一檔市場前綴錯誤的股票（`3105`／穩懋實際是上櫃，這裡故意用 `tse_` 查詢驗證邊界情況）：

```
GET https://mis.twse.com.tw/stock/api/getStockInfo.jsp?ex_ch=tse_2330.tw|tse_3105.tw|tse_1101.tw&json=1&delay=0
...
第二筆（`3105`）在對應位置回空殼物件，第一、三筆正常回值——確認可以用**位置對應**（而非
從回應內容反查股號）安全解析批次結果。這個 `|` 分隔的批次語法同樣**非官方文件化**（跟
單檔查詢的 `ex_ch` 一樣，`openapi.twse.com.tw` 的 Swagger 未收錄），沿用本 ADR「決策」段
既有的風險評估與速率限制假設，不視為新的風險層級，故不另開 ADR，只在此追加記錄。

**影響**：`app/clients/stock_price_client.py` 新增 `get_quotes()` 批次方法，`net_worth_service.py`
改成對股票整批查詢（不論持有幾檔股票，最多 2 次外部呼叫：`tse_` 一次、查無資料的再整批試
`otc_` 一次），取代逐檔序列 / 平行發起皆會被同一限速鎖排隊的寫法。美股（Yahoo Finance chart
API，路徑帶單一代號，非設計成批次）與貴金屬（固定只有 XAU/XAG 兩種符號，本來就不是瓶頸）
不受影響，維持原有查詢方式。
