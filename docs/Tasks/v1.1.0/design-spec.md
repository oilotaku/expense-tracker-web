...
**行動端（`< md`）**：底部固定 Bottom Nav（5 格，`min-h-[64px]`，icon + 極簡 label，中間為突出的 FAB「+」）：首頁 / 交易 / **(+)** / 預算 / 更多。「更多」開 `<Dialog>` 型選單（非新路由）列出：週期性、資產、分類、帳戶、外觀切換（`→ §2.7`）、設定、登出。頂部 Header 精簡：頁面標題 + 必要時的期間選擇器（Dashboard）。
