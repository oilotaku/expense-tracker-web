...
> 狀態：進行中（已完成 13/18）　盲點掃描：propose 拆解前已用 AskUserQuestion 解決三個缺口（負債輸入方式、分類/標籤結構、週期性交易月底夾日規則），已回寫 `propose-v1.0.0.md`。報價資料源仍未定案 → task-011 / task-012 為調查 spike，其結論會影響 task-013 / task-014 的實作細節（非阻塞拆解，可先拆）。
...
| 008 | 週期性交易前端設定頁 | in_progress |  | task-007 | `frontend/src/lib/api/recurringApi.ts`、`frontend/src/app/recurring/page.tsx` | sub-agent |
...
| 010 | 預算前端頁（設定 + 進度顯示） | in_progress |  | task-009 | `frontend/src/lib/api/budgetsApi.ts`、`frontend/src/app/budgets/page.tsx` | sub-agent |
...
| 016 | 總資產 / 總負債彙總 API | in_progress |  | task-002, task-013, task-014, task-015 | `backend/app/services/net_worth_service.py`、`backend/app/api/v1/net_worth.py`、`backend/app/schemas/net_worth.py`、`backend/tests/api/test_net_worth.py` | sub-agent |
