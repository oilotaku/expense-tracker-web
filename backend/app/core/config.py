...
# 內部排程觸發 API（POST /internal/recurring/run）的驗證密鑰，只供 development 預設值，同上限制。
INTERNAL_TRIGGER_SECRET_DEVELOPMENT_DEFAULT = "development-only-internal-secret-do-not-use-dev"
...
class Settings(BaseSettings):
    ...
    # host 端 cron/systemd timer 呼叫 POST /internal/recurring/run 時用（→ CICD-086 排程慣例）
    INTERNAL_TRIGGER_SECRET: str = Field(
    ...
    def _fail_fast(self) -> Settings:
        ...
        for name in ("JWT_SECRET_KEY", "INTERNAL_TRIGGER_SECRET", "DATABASE_URL"):
        ...
        if (
            ...
            and self.INTERNAL_TRIGGER_SECRET == INTERNAL_TRIGGER_SECRET_DEVELOPMENT_DEFAULT
        ):
            raise ValueError(
                f"APP_ENV={self.APP_ENV} 但 INTERNAL_TRIGGER_SECRET 仍為 development 預設值"
            )
