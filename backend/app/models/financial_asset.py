"""金融資產：台股（股號 + 張/股）、美股（代號 + 股）與貴金屬（品項 + 兩/錢），歸屬使用者。
...
class FinancialAsset(BaseModel):
    ...
    # "stock"（台股）| "us_stock"（美股）| "metal"（→ ck_financial_assets_asset_type）
    asset_type: Mapped[str] = mapped_column(String(10), nullable=False)
    # 台股存股號（例："2330"）；美股存代號（例："AAPL"）；貴金屬存品項名稱（例："黃金"）
    name: Mapped[str] = mapped_column(String(100), nullable=False)
    # 原始輸入量與單位：台股「張」或「股」、美股僅「股」、貴金屬「兩」或「錢」
    ...
    # 換算後基本單位數量：stock/us_stock → 股數；metal → 錢數
    ...
    __table_args__ = (
        CheckConstraint(
            "asset_type IN ('stock', 'us_stock', 'metal')", name="ck_financial_assets_asset_type"
        ),
        CheckConstraint(
