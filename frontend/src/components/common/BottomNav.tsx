...
/**
 * 行動端底部固定導覽（design-spec §3.1：5 格 `min-h-[56px]`，中間為突出 FAB，`md:hidden`
...
export function BottomNav({ items, moreItems, onAddClick, onLogout }: BottomNavProps): ReactNode {
  ...
  return (
    <>
      <nav
        className="fixed inset-x-0 bottom-0 z-30 flex h-14 items-stretch border-t border-border bg-surface md:hidden"
        aria-label="行動導覽"
      >
        {leftItems.map((item) => (
